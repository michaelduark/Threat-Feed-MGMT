...
#whitelist_feeds_path = os.path.join(base_path, "whitelist_feeds.txt")
...
# Determine the max number of CPUs and utilize N-1 so that a CPU is free to handle background processes
...
def download_file(url):
    try:
        ...
        file_name = f"{timestamp}_THREAT_{fqdn}_{file_name}"
        # Download the file
...
with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
    # Download the files using threads
    ...
    for future in concurrent.futures.as_completed(futures):
        try:
            ...
            logging.error(f"An error occurred while downloading a file: {e}")
...
# Number of seconds in 32 days
SECONDS_IN_32_DAYS = 32 * 24 * 60 * 60

# Get the current time
current_time = time.time()

# Define a function to delete a file and return its name
...
directories = [logs_archived_path, raw_archived_path]

# Loop over the directories and delete the old files in each directory
for directory in directories:
    # Use a list comprehension to get the list of old files
    old_files = [file for file in os.listdir(directory)
                 if os.path.isfile(os.path.join(directory, file))
                 and current_time - os.stat(os.path.join(directory, file)).st_ctime >= SECONDS_IN_32_DAYS]

    # Use a ThreadPoolExecutor to delete the old files in parallel
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        results = executor.map(delete_file, old_files, [directory] * len(old_files))

    # Log the deleted files
    for result in results:
        if result is not None:
            logging.info(f"File {result} deleted successfully from {directory}.")